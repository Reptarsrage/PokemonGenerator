...
namespace PokemonGenerator.Windows
{
    ...
    public partial class MainWindow : WindowBase
    {
        ...
        private bool ValidateTopSection()
        {
            if (CheckIfFileExistsAndAssignImage(TextProjN64Location, ImageProjN64Location, ".exe"))
            {
                ...
                if (File.Exists(ini))
                {
                    ...
                    _inRageConfigRepository.FileName = ini;
                    var tup = _inRageConfigRepository.GetRomAndSavFileLocation(1);
                    var tup2 = _inRageConfigRepository.GetRomAndSavFileLocation(2);
                ...
                if (File.Exists(cfg))
                {
                    _ip64ConfigRepository.FileName = cfg;
                }
        ...
        private void BackgroundPokemonGeneratorDoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                _inRageConfigRepository.ChangeSavLocations(args.Options.PlayerOne.OutputSaveLocation, args.Options.PlayerTwo.OutputSaveLocation);
            }

            // Get Recent N64 Rom
            try
            {
                var rom = _ip64ConfigRepository?.GetRecentRom() ?? "";
